...
{# Son teslim metni teslim_kilit'ten gelir (main.py → son_teslim): Teslim
   sayfasıyla aynı kaynak, saat dahil. Takvimdeki teslim satırı başka güne
   kayarsa derleme hata verir. #}
{% set _teslim = son_teslim() %}
...
{% if _teslim %}<p class="staj-lede__meta"><span class="staj-lede__meta-label">Rapor teslim son günü</span><strong class="staj-lede__meta-value">{{ _teslim }}</strong><a class="staj-lede__meta-link" href="teslim/">Belgeleri yükle<span aria-hidden="true"> →</span></a></p>{% endif %}
...
{# Şerit + tarih listesi. GEOMETRİ main.py içinde hesaplanır (test: python3 main.py).
   ...
   pencereleri bilerek satır açmaz (gerekçe: main.py başlığı). Şerit her genişlikte
   görünür; dar ekranda .staj-yearbar__scroll yatay kaydırır, legend dışarıda
...
- Şu belgeleri hazırlayın: **Staj Sicil Formu**, Öğrenci Staj Anketi, Staj Raporu ve (varsa) Cumartesi Yazısı, Ödeme Dekontu veya Bordro ve EK-1.
