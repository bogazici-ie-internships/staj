...
{# Deadline text comes from teslim_kilit (main.py → son_teslim): the same
   source as the Submission page, hour included. #}
{% set _deadline = son_teslim(true) %}
...
{% if _deadline %}<p class="staj-lede__meta"><span class="staj-lede__meta-label">Internship report deadline</span><strong class="staj-lede__meta-value">{{ _deadline }}</strong><a class="staj-lede__meta-link" href="teslim/">Upload documents<span aria-hidden="true"> →</span></a></p>{% endif %}
...
- Prepare the **Trainee Evaluation Form**, Student Internship Survey, Internship Report and, if applicable, a Saturday Work Letter and a payment receipt or payslip with EK-1.
