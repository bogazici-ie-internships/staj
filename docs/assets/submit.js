/* Teslim formu — TEK mantık kaynağı (TR + EN).
 *
 * Daha önce bu ~360 satır teslim.md ve teslim.en.md içinde satır içi <script>
 * olarak İKİ KEZ duruyordu. İkisi birbiriyle uyumluydu, ama bir doğrulama
 * kuralını düzeltmek iki dosyada birden düzeltmeyi gerektiriyordu; atlanan bir
 * düzenleme yalnızca TEK dilde sessizce bozulmaya yol açardı. Artık mantık
 * burada, sayfaya özgü olan her şey dışarıdan geliyor:
 *
 *   - Dönem/kampanya değerleri  -> #sufForm üzerindeki data-* öznitelikleri
 *                                  (settings.yml'den Jinja ile basılır)
 *   - Ekranda görünen metinler  -> aşağıdaki STRINGS tablosu, dile göre
 *                                  (dil: data-locale)
 *
 * Statik form etiketleri ("Ad Soyad", "Zorunlu Belgeler" vb.) içerik olduğu
 * için markdown dosyalarında kalır; buraya yalnız JS'in ÜRETTİĞİ metinler taşındı.
 *
 * Site genelinde yüklenir (yearbar.js gibi); Teslim sayfası dışında #sufForm
 * bulunmadığı için erkenden çıkar.
 */
(function () {
  "use strict";

  // Belge başına MB sınırları — Code.gs içindeki FILE_MAP ile aynı olmalı.
  ...
  function init() {
    ...
    var TERM             = d.term || "";                 // sözleşme karşılaştırması (çevrilmez)
    var TERM_DISPLAY     = d.termDisplay || d.term || ""; // ekranda görünen
    // NOT: data-* değerleri her zaman METİNDİR. sameContract() sıkı eşitlik
    // kullandığı için sayısal alanları burada Number()'a çevirmek ZORUNLU.
    ...
    // Türkiye saatiyle (+03:00) — izleyicinin saat diliminden bağımsız.
    function fmtDeadline(ms) {
      var dt = new Date(ms + 3 * 3600000);   // +03:00'e kaydır, sonra UTC alanlarını oku
    ...
    // Yükleme sürerken sekmeyi kapatmaya çalışana tarayıcının kendi
    // "ayrılmak istediğinize emin misiniz?" uyarısını çıkarır.
    // NEDEN: dosyalar sunucuya ulaştıysa Apps Script istemci gitse de işlemi
    // TAMAMLAR. Öğrenci onay ekranını göremediği için teslimi başarısız sanıp
    // yeniden yükler; requestId sayfayla birlikte gittiği için sunucudaki
    // idempotency devreye giremez ve MÜKERRER kayıt oluşur.
    // isUploading her seferinde okunur → yükleme bitince kendiliğinden susar.
    window.addEventListener("beforeunload", function (ev) {
      if (!isUploading) return;
      ev.preventDefault();
      ev.returnValue = "";   // eski tarayıcılar bunu ister
      return "";
    });
    ...
    form.addEventListener("submit", function (ev) {
      ...
      Promise.all(keys.map(function (k) { return readB64(picked[k]); })).then(function (b64s) {
        ...
        // Teslim tamamlandı: beforeunload uyarısı SUSMALI. Aksi halde öğrenci
        // onayı gördükten sonra sekmeyi kapatırken gereksiz yere uyarı alır.
        isUploading = false;
    ...
    function checkContract() {
      ...
      // Üst banner (gate) aynı mesajı tekrar yazmaz; yalnızca doğrulama BAŞARISIZ olursa görünür.
      if (!contractReady) {
        gate.style.display = "none";
